...
\begin{enumerate}
    ...
    \item Insert \textbf{Lambda Calculus}. $\lambda$-Calculus is a system for devising computation. The formality of this system allows programming languages to be studied at a higher level. //For those interested, lambda calculus is a cartesian closed category, and so the study of a lambda calculus inherits many tools from many different mathematical areas.// Lambda calculus has been proven to be Turing Complete, and so any programming language that is Turing Complete can be simulated using $\lambda$-Calculus.  
...
\item Now onto the second rule about $\lambda$-abstractions. Directly above we just created these two variables $x$ and $y$ but we never actually created the functions $f$ and $g$. Well what good are variables if nothing is using them. Insert $\lambda$-abstractions, the $f$ in the rule definition is essentially a function, and so $\lambda x.f$ is the function $\mathbf{f}$ acting upon the variable $\mathbf{x}$. 
\begin{center}
...
\begin{center}
$(\lambda x.x + x)y$ $\quad$ $\rightarrow$ $\quad$ $f(x) = 2x$ $\quad$ $\rightarrow$ $\quad$ $f(y) = 2y$
\end{center}
...
\begin{center}
$(\lambda x.y)(z)$ $\quad = \quad$ $(\lambda x.y)[x:=z]$ $\quad \Rightarrow \quad$ $f(x) = y$ $\quad \rightarrow \quad$ $f(z) = y$
\end{center}
\end{itemize}
